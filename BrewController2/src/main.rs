...
// Import the I2cBusLinux module from the Drivers directory
mod Drivers {
    pub mod I2cBusLinuxLib;
    pub mod I2cBusTraitLib;
}

use Drivers::I2cBusLinuxLib;
use I2cBusLinuxLib::I2cBusLinux;
fn main() {
    let i2c_bus = I2cBusLinux::new(1, "MyI2CBus");

    println!("I2C Bus Name: {}", i2c_bus.get_name());

    let mut data = [0; 4];
    let status = i2c_bus.read_data(0x50, &mut data);
    println!("Read Data Status: {:?}", status);
    println!("Read Data: {:?}", data);

    let mut byte = 0;
    let status = i2c_bus.read_byte(0x51, &mut byte);
    println!("Read Byte Status: {:?}", status);
    println!("Read Byte: {:?}", byte);

    let write_data = [0x12, 0x34, 0x56, 0x78];
    let status = i2c_bus.write_data(0x52, &write_data, 4, 100);
    println!("Write Data Status: {:?}", status);

    let status = i2c_bus.write_byte(0x53, 0xAA);
    println!("Write Byte Status: {:?}", status);
}
