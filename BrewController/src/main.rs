...
use SimpleKalmanFilterLib::SimpleKalmanFilter;
mod BrewControllerLib;
use BrewControllerLib::BrewController;
mod LoggerLib;
use LoggerLib::Logger;


fn main() {
    let brew_controller = BrewController::get_instance();
    let mut locked_brew = brew_controller.write().unwrap();
    locked_brew.Initialize();
    

    //    // Initialize the static Mutex if it hasn't been initialized yet
    //    unsafe {
    //     if MyBrewController.is_none() {
    //         MyBrewController = Some(BrewController::get_instance().clone());
    //     }
    //     // Now you can safely lock the Mutex when you need to access it
    // let brew_controller = unsafe { MyBrewController.as_ref().unwrap() };
    // let locked_brew = brew_controller.lock().unwrap();
    // }
}
