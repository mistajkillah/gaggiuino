...
// Set time since system start
  ...
  {
    return static_cast<uint32_t>(std::chrono::duration_cast<std::chrono::milliseconds>(std::chrono::system_clock::now() - lastReadTime_pres).count());
  }
    float inline getTimeSincelastRead_temp()
  {
    return static_cast<uint32_t>(std::chrono::duration_cast<std::chrono::milliseconds>(std::chrono::system_clock::now() - lastReadTime_temp).count());
  }
