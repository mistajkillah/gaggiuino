...
//  BASE CONSTRUCTOR
//
 ADS1X15::ADS1X15(I2cBus * bus, uint8_t devAddr, const char * name): _bus(bus), _address(devAddr), _name(name)
{
  reset();
}
...
int16_t ADS1X15::_readADC(uint16_t readmode)
{
  ...
  {
    while ( isBusy() ) std::this_thread::yield();   //  wait for conversion; yield for ESP.
  }
  else
  {
...
bool ADS1X15::_writeRegister(uint8_t address, uint8_t reg, uint16_t value)
{
  ...
  uint8_t txData[3]={reg,(uint8_t)((value>>8)&0x0FF), (uint8_t)(value & 0x0FF)};
  if(GenericDriverStatus_Success != (status = _bus->writeData(address,txData,3)))
  {
...
uint16_t ADS1X15::_readRegister(uint8_t address, uint8_t reg)
{
  uint8_t data[2];
  GenericDriverStatus status = GenericDriverStatus_Success;
  if(GenericDriverStatus_Success != (status = _bus->writeReadData(address,&reg,1,data,2)))
  {
...
//  ADS1013
//
ADS1013::ADS1013(I2cBus * bus,  const char * name,uint8_t devAddr):ADS1X15(bus,devAddr,name)
{
...
//  ADS1014
//
ADS1014::ADS1014(I2cBus * bus,  const char * name,uint8_t devAddr):ADS1X15(bus,devAddr,name)
{
...
//  ADS1015
//
ADS1015::ADS1015(I2cBus * bus,  const char * name,uint8_t devAddr):ADS1X15(bus,devAddr,name)
{
...
//  ADS1113
//
ADS1113::ADS1113(I2cBus * bus,  const char * name,uint8_t devAddr):ADS1X15(bus,devAddr,name)
{
...
//  ADS1114
//
ADS1114::ADS1114(I2cBus * bus, const char * name,uint8_t devAddr):ADS1X15(bus,devAddr,name)
{
...
//  ADS1115
//
ADS1115::ADS1115(I2cBus * bus,  const char * name,uint8_t devAddr):ADS1X15(bus,devAddr,name)
{
