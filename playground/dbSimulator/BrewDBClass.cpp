...
BrewDBClass& BrewDBClass::getInstance() {
    static BrewDBClass instance; // Guaranteed to be destroyed once
    return instance;
}

void BrewDBClass::Initialize() {
    std::lock_guard<std::mutex> lock(dbMutex);
    InitializeDatabase();
}
    std::string  BrewDBClass::RetrieveLastSensorStateDataAsString() {
        std::lock_guard<std::mutex> lock(dbMutex);
    ...
    std::string BrewDBClass::RetrieveLastSensorStateSnapshotDataAsString() {
        std::lock_guard<std::mutex> lock(dbMutex);
...
void BrewDBClass::InsertSensorStateData(const SensorState& data) {
    std::lock_guard<std::mutex> lock(dbMutex);
...
void BrewDBClass::InsertSensorStateSnapshotData(const SensorStateSnapshot& data) {
    std::lock_guard<std::mutex> lock(dbMutex);
...
void BrewDBClass::StartSimulator() {
    std::thread simulator(&BrewDBClass::SimulatorThread, this);
    simulator.detach();
}

void BrewDBClass::InitializeDatabase() {
...
void BrewDBClass::SimulatorThread() {
